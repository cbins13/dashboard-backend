...
const run = async () => {
    ...
    try {
        ...
        const insertedRecords = await withTargetSchema(sequelize, targetSchema, (transaction) =>
            Person.bulkCreate(persons, { transaction })
        );

        console.log(
            `Inserted ${insertedRecords.length} sample person records into schema ${targetSchema}.`
        );
    } catch (error) {
        console.error('Sample data insert failed:', error.message);
        process.exitCode = 1;
    } finally {
